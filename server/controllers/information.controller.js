  ...
  try {
    ...
    for (classTeacher of classTeachers) {
      ...
      for (room of classRooms) {
        const roomSchedule = schedules.find((item) => item.classRoom === room);

        if (!checkConflictSchedule(schedule, roomSchedule.schedule, subject)) {
          throw new Error(
...
module.exports.getAdminChart = async (req, res) => {
  try {
    const grade10 = await Parent.countDocuments({
      isDeleted: false,
      grade: 10,
    });
    const grade11 = await Parent.countDocuments({
      isDeleted: false,
      grade: 11,
    });
    const grade12 = await Parent.countDocuments({
      isDeleted: false,
      grade: 12,
    });
    ...
    const data = {
      pieChart: { grade10, grade11, grade12 },
      columnChart,
    };
...
module.exports.teacherGetStudentOff = async (req, res) => {
  try {
    ...
    if (!teacher.mainTeacherOfClass || teacher.mainTeacherOfClass.length === 0)
...
module.exports.getClassTranscript = async (req, res) => {
  try {
    ...
    data = data.sort((st1, st2) =>
      st1.studentName.toLowerCase() < st2.studentName.toLowerCase() ? -1 : 1
    );
