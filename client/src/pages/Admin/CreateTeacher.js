  ...
  const getData = () => {
    ...
    if (id) {
      ...
      getTeacherData(id).then((res) => {
        ...
        props.setFieldValue("mainTeacherOfClass", data.mainTeacherOfClass);
      ...
      <Row>
        ...
        <Col md={6}>
          ...
          <FormGroup className="d-flex flex-column">
            ...
            <div>
              <AllClassSelected
                isDisabled={!props.values.mainTeacher || teacherId}
                onChange={(e) => {
...
export default withFormik({
  ...
  validationSchema: yup.object().shape({
    ...
    gender: yup.boolean().required("Giới tính không được để trống").nullable(),
    email: yup
      .string()
    ...
    teacherOfClass: yup
      ...
      .min(0, "Các lớp đang dạy không được để trống"),
  }),
  handleSubmit: async (values, { props }) => {
    try {
      ...
      const res = await (id
        ? (teacherId ? updateProfile : updateTeacher)(values, id, "teacher")
