  ...
  const renderModalMarkOff = () => (
    ...
    >
      ...
      <div className="text-center">
        <Button color="success" className="mr-2" onClick={() => markOff(true)}>
          Có phép
        </Button>
  ...
  const getData = async () => {
    try {
      const res = await (role === "admin"
        ...
        : props.teacherGetAllStudent(searchString, optionClass, currentPage));
  ...
  const renderModal = () => {
    ...
    return (
      <ViewModal
        ...
        title={`Thông tin ${parent.position === "father" ? "bố" : "mẹ"}`}
        // viewOnly
      >
        <p>Tên: {parent.name || ""}</p>
          ...
          <>
            <Col md={3}>
              <FilterSelected
                ...
                isDisabled={role === "admin" && filterClassStudent.length === 0}
              />
            </Col>
            ...
            <Table bordered striped hover size="sm" responsive>
              <thead>
                <tr>
                  ...
                  {props.isMainTeacher && (
                    <th>
                      <div
                        ...
                        title="Tổng kết điểm"
                        onClick={() =>
                          history.push(`/confirmTranscript/${props.classRoom}`)
                        }
                      >
                  ...
                  {(props.isComponent || role === "admin") && <th></th>}
                </tr>
              </thead>
              <tbody>
                {data.map((student, index) => (
                  <tr key={index}>
                    ...
                    <td>{renderParent(student.father, "father")}</td>
                    <td>{renderParent(student.mother, "mother")}</td>
                    ...
                    {(props.isMainTeacher || role === "admin") && (
                    ...
                    {props.isComponent && (
                      <td>
                        <div
                          className="text-center"
                          title={
                            checkMarkOff(student.dayOff)
                              ? "Đánh dấu nghỉ học"
                              : checkPermissionOff(student.dayOff)
                              ? "Nghỉ học có phép"
                              : "Nghỉ học không phép"
                          }
                          onClick={() => onMarkOff(student)}
                        >
                          {checkMarkOff(student.dayOff) ? (
                            <i
