...
function GradeAndClass(props) {
  ...
  const createClass = () => {
    ...
    createClassRoom({ grade, teacherId, className })
      .then(() => {
        renderNoti({
          type: "success",
          title: "Thành công",
  ...
  const renderModalCreateClass = () => (
    ...
    >
      <Row>
        ...
        <Col md={6} className="pt-2 mx-auto">
      ...
      <Row className="mb-2">
        <Col md={!isPhone && 8} className="d-flex align-items-start">
          <h5 className="flex-grow-1">
            QUẢN LÝ LỚP HỌC {props.year && `${props.year}-${props.year + 1}`}
          </h5>
          {!isPhone && (
            <>
              <Button
                color="success"
                className="mr-2"
                onClick={() => toggleCreateClass(!isOpenCreateClass)}
              >
                Tạo lớp học
              </Button>
            ...
            <Table bordered responsive>
              ...
              <tbody>
                {grades.map((grade, index) =>
                  grade.classRoom && grade.classRoom.length > 0 ? (
                    <React.Fragment key={index}>
                      <tr>
                        <th rowSpan={grade.classRoom.length}>{grade.grade}</th>
                        <td>{grade.classRoom[0]?.room}</td>
                        <td>
                          {grade.classRoom[0] &&
                          grade.classRoom[0].mainTeacher ? (
                            <NewTabLink
                              to={`/user/teacher/edit/${grade.classRoom[0].mainTeacher._id}`}
                              title={grade.classRoom[0].mainTeacher.name}
                            />
                          ) : (
                            ""
                          )}
                          ...
                          <div
                            ...
                            onClick={() =>
                              history.push("/students", {
                                optionClass: grade.classRoom[0].room,
                              })
                            }
                          >
                        ...
                        <td className="text-center">
                          <div
                            ...
                            onClick={() => {
                              setCurrentClass(grade.classRoom[0].room);
                              toggle(!isOpen);
                            }}
                          >
                        ...
                        <td>
                          <DeleteBtn
                            onClick={() => {
                              deleteClass(grade.classRoom[0].room);
                            }}
                          />
                        </td>
                      </tr>
